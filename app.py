...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///blog.db' #название blog сами придумываем
...
@app.route('/new_post', methods = ['GET', 'POST'])  #Есть 2 метода в интернете GET и POST(GET - что-то получить от сайта, POST - что-то загружать на сайт) по умолчанию GET
def new_post():
